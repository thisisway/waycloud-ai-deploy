...
- **P4 revisado — domínio de prévias: `*.preview.wayleads.com.br`** (escolha do cliente). Diferente do waycloud.com.br, o wayleads.com.br não hospeda o WHMCS. Situação atual do domínio: DNS na Cloudflare (mesmos nameservers do waycloud), site no ar (A 177.53.143.53, servidor cPanel "Server PRO BR") e e-mail próprio (MX/SPF). **Risco residual R15**: cookies gravados por uma prévia com escopo `.wayleads.com.br` podem afetar o site e a área de login do próprio wayleads. Aceito enquanto o wayleads.com.br não tiver login/sessão sensível; migrar para um domínio exclusivo de prévias se isso mudar.
- 🔶 Certificado: o wildcard universal da Cloudflare cobre só `*.wayleads.com.br`, não `*.preview.wayleads.com.br`. Usar registro DNS-only (nuvem cinza) e emitir o wildcard por DNS-01 (Let's Encrypt) com token da Cloudflare restrito à zona wayleads.com.br (permissão só DNS:Edit). Confirmar como o Traefik do Easypanel (campo `wildcard` nos domínios) faz DNS-01.
