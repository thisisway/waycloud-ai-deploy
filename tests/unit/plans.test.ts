...
function setup(behavior: () => Promise<AddonPlan[]>) {
  ...
  const addon = { plans: () => (state.calls++, state.behavior()), createCheckout: async () => { throw new Error("unused"); }, registerCheckout: async () => { throw new Error("unused"); }, updateServiceDomain: async () => {} } as AddonClient;
